...
class Pgp3layer(object):
    def __init__(self, numvis, numnote, numfac, numvel, numvelfac, numacc, numaccfac, numjerk, seq_len_to_train, seq_len_to_predict, output_type='real', coststart=4, vis_corruption_type="zeromask", vis_corruption_level=0.0, numpy_rng=None, theano_rng=None):
        self.numvis = numvis
        ...
        self.wxf_left = theano.shared(value = self.numpy_rng.normal(size=(numvis+numnote, numfac)).astype(theano.config.floatX)*0.01, name='wxf_left')    # U
        self.wxf_right = theano.shared(value = self.numpy_rng.normal(size=(numvis+numnote, numfac)).astype(theano.config.floatX)*0.01, name='wxf_right')  # V
        self.wv = theano.shared(value = self.numpy_rng.uniform(low=-0.01, high=+0.01, size=(numfac, numvel)).astype(theano.config.floatX), name='wv')     # W
        ...
        self.params = [self.wxf_left, self.wxf_right, self.wv, self.wvf_left, self.wvf_right, self.wa, self.waf_left, self.waf_right, self.wj, self.bx, self.bv, self.ba, self.bj]
        ...
        for t in range(4, self.seq_len_to_predict):
            ...
            self._accs[t] = T.dot(T.dot(T.nnet.sigmoid(self._prejerks[t]), self.wj.T) * T.dot(self._preacc123, self.waf_left), self.waf_right.T) + self.ba
            self._vels[t] = T.dot(T.dot(self._accs[t], self.wa.T)*T.dot(self._prevel23,self.wvf_left), self.wvf_right.T)+self.bv
        ...
        if self.output_type == 'binary':
            self._prediction_for_training = T.concatenate([T.nnet.sigmoid(pred[:,:self.numvis]) for pred in self._recons_with_notebook[self.coststart:self.seq_len_to_train]], 1)
        else:
            self._prediction_for_training = T.concatenate([pred[:,:self.numvis] for pred in self._recons_with_notebook[self.coststart:self.seq_len_to_train]], 1)

        print self.output_type
        if self.output_type == 'real':
            self._cost = T.mean((self._prediction_for_training - self.inputs[:,self.coststart*self.numvis:self.seq_len_to_train*self.numvis])**2)
        elif self.output_type == 'binary':
            self._cost = -T.mean(self.inputs[:,self.coststart*self.numvis:self.seq_len_to_train*self.numvis]*T.log(self._prediction_for_training) 
                                    + 
    ...
    def predict(self, seedframes, seq_len=10):
        ...
        for i in range(seedframes.shape[1]/self.numvis, seq_len):
        ...
        preds = numpy.concatenate((seedframes[:,:self.numvis*4], numpy.zeros((numcases,(seq_len-4)*self.numvis),dtype="float32")), 1)
    ...
    """
    def updateparams_(self, newparams):
        def inplaceupdate(x, new):
            x[...] = new
            return x
        ...
        for p in self.params:
            pshape = p.get_value().shape
            pnum = numpy.prod(pshape)
            newvalue = newparams[paramscounter:paramscounter+pnum].reshape(*pshape)
            if p == self.wxf_left or p == self.wxf_right or\
                p == self.wvf_left or p == self.wvf_right or\
                p ==self.waf_left or p == self.waf_right:
                newvalue = newvalue.T
            p.set_value(inplaceupdate(p.get_value(borrow=True), ), borrow=True)
            paramscounter += pnum 

    def load_(self, filename):
        self.updateparams_(numpy.load(filename))
    """
