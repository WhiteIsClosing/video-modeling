...
logInfo = LogInfo('_LOG.txt')
logInfo.mark('continue training ...')
...
features_numpy = features_numpy[ \
    numpy.random.permutation(features_numpy.shape[0])]

features_theano = theano.shared(features_numpy)
...
for epoch in xrange(max_epoch):
    ...
    if (epoch+1) % save_epoch == 0:
        recons = model.predict(features_numpy)
        model.save(models_path + 'continue_model')
