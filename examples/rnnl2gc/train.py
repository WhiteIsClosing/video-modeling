...
from utils.gcParams import GCParams
from utils.gcParamsValue import GCParamsValue
from utils.log import *

from rnn.rnnl2gc import RNNL2GC
...
# INITIALIZATION
tic = clock()
model = RNNL2GC(frame_dim, frame_dim*2, hidden1_size, hidden2_size, numvel)
toc = clock()
...
while (1):
  ...
  for i in xrange(numseqs_train):
    ...
    cost_train += model.train(rawframes_train[rows, :], \
                  vels_train[rows, :], labels_train[rows, :], lr)
  ...
  for i in xrange(numseqs_test):
    rows = range(i*seq_len, (i+1)*seq_len)
    cost_test += model.getCost(rawframes_test[rows, :], \
                  vels_test[rows, :], labels_test[rows, :])
