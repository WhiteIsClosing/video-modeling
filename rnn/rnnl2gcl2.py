...
class RNNL2GCL2(object):
    
    def __init__(self, dimx, dimy, dimh1, dimh2, dimvel):
        '''
        ...
        vels :: velocity mapping unit data
        '''
        ...
        self.Wvel  = theano.shared(0.2 * numpy.random.uniform(-1.0, 1.0,\
                   (dimvel, dimh1)).astype(theano.config.floatX))
        ...
        def recurrence(x_t, vel_t, h2_tm1):
            h1_t = T.nnet.sigmoid(T.dot(x_t, self.Wx) + T.dot(vel_t, self.Wvel)\
                                   + self.bh1)
            h2_t = T.nnet.sigmoid(T.dot(h1_t, self.Wh1)+T.dot(vel_t, self.Wvel)\
             + T.dot(h2_tm1, self.Wh2) + self.bh2)
