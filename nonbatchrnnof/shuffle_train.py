...
labels_train_ = numpy.reshape(labels_train, (numframes_train, frame_dim*2))

rawframes_train_sf = \
numpy.reshape(features_train_numpy, (numseqs_train, seq_dim))
labels_train_sf = numpy.reshape(labels_train, (numseqs_train, seq_dim*2))
...
while (1):
  ...
  rawframes_train = \
  numpy.reshape(rawframes_train_sf, (numframes_train, frame_dim))
  labels_train = numpy.reshape(labels_train_sf, (numframes_train, frame_dim*2))
  ...
  if (epoch % save_epoch == 0):
    ...
    # predictions
    for i in xrange(numseqs_train):
      preds_train[i*seq_len:(i+1)*seq_len, :] = model.predict(rawframes_train_[i*seq_len:(i+1)*seq_len, :])
